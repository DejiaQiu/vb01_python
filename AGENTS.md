...
- 当前阶段统一主判只保留钢丝绳主链，橡胶圈规则保留代码但暂不参与主流程决策。
- 通用异常门、钢丝绳规则都优先采用“少量特征证据值 + 命中数”的极简规则，不再堆复杂加权评分。
- 钢丝绳规则应主要依赖横向比例、横向主频偏移、横向低频占比等频域与横向特征。
- 批诊断结果应额外做连续窗口确认；当连续窗口都命中钢丝绳观察级规则时，可提升为已确认的钢丝绳连续异常。
