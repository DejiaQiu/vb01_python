...
def build_feature_pack(rows: list[dict[str, str]], axis_mapping: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    feature = {
        ...
        "corr_xy_abs": float(abs(corr_xy)),
        "corr_xz": float(corr_xz),
        "corr_xz_abs": float(abs(corr_xz)),
        ...
        # 这 3 个比值用于“运行态/活跃态”判断，让不同电梯先在各自健康基线附近做比较。
        "a_activity_ratio": float(a_rms_ac / max(abs(a_mean), 1e-3)),
        "g_activity_ratio": float(g_std / max(abs(g_mean), 0.05)),
        "activity_peak_ratio": float(max(lateral_spectrum["peak_ratio"], vertical_spectrum["peak_ratio"])),
