...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--sample-hz", type=float, default=40.0, help="轮询采样频率")
    parser.add_argument("--detect-hz", type=int, default=40, help="设备检测周期，写入寄存器 0x65")
    ...
    parser.add_argument("--reg-count", type=int, default=13, help="循环读取寄存器数量（40Hz 默认推荐 13）")
    parser.add_argument("--emit-mode", choices=["new", "fixed"], default="fixed", help="new=仅写新帧；fixed=按固定频率写出（默认）")
    parser.add_argument("--emit-hz", type=float, default=40.0, help="fixed 模式输出频率")
    parser.add_argument(
        "--sdk-default",
        action="store_true",
